...
const Creadores = ({ creadores }) => {
    ...
    return (
        <div className="creadores-card">
            ...
            <h4>Profesión</h4>
            <ul>{c.profesion.map((p, i) => <li key={i}>{p}</li>)}</ul>

            <h4>Roles en la serie</h4>
            <ul>{c.rolesEnLaSerie.map((r, i) => <li key={i}>{r}</li>)}</ul>

            <h4>Otras obras</h4>
            <ul>{c.obrasAdicionales.map((o, i) => <li key={i}>{o}</li>)}</ul>

            <h4>Historia</h4>
            <p>{creadores.historia}</p>

            <h4>Curiosidades</h4>
            <ul>{creadores.curiosidades.map((cur, i) => <li key={i}>{cur}</li>)}</ul>

            <h4>Premios y reconocimientos</h4>
            <ul>{creadores.premiosYReconocimientos.map((p, i) => <li key={i}>{p}</li>)}</ul>
        </div>
    );
};
